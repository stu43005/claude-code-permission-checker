...
- **`rules/`**：`types.ts`（`CommandRule`/`RuleContext`/`RuleVerdict` + `allow()`/`ask()`/`deny()`，
  ...
  **本次新增四條規則**：`base64.ts`（走 `CommandSpec`：`-d`/`--decode`/`-i`/`--ignore-garbage` 不吃值、
  ...
  `package.json` 並印出父層專案內容，且沒有操作元可供本工具做範圍檢查）。
...
### 四條中央前置規則（於 classify.ts；個別 rule 不需重複處理）
...
- **`resolvePath` 對未加引號的 leading tilde 必須先展開**：shell home 一律用 `HOME`，**不可用
  `resolveHome`**——後者在 Windows 優先 `USERPROFILE`，是 settings 端的家目錄語義，不是指令執行時
  bash 展開 `~` 所依據的環境變數；兩者混用會讓 `cat ~/.ssh/id_rsa` 這類指令在 `HOME`/`USERPROFILE`
  分歧時被誤判。`resolvePathValue` 對以 `~` 開頭的字串一律 fail-closed（不可解析 → ask，不臆測
  展開結果）。引號形態（`"~/x"`）必須維持相對語義、不可一併擋掉——它不會被 bash 展開，是相對於
  cwd 的字面路徑。另 `C:Windows` 這種 Windows 磁碟相對形態語義有歧義（可能是磁碟根、也可能是
  該磁碟當前目錄下的 `Windows`，且無正當寫法可用來表達使用者真正想要的路徑），一律視為超出範圍。
...
三組一致施加（raw 永遠保留 → 不弱化任何既有命中）。正規化由 `scope.ts` 的 `canonicalizeExecPath` 對**執行檔
token** 做純詞法處理：展開 `~`/`~/`（home 已知時）、折疊中段 `//`、移除 `.` 段；含 `..` 段、前導 `//`（UNC）、
塌成空/裸根、含 `/` 卻塌成裸名（path-exec vs PATH-lookup）者一律留字面（fail-closed、symlink 安全）。**指令側
固定以 `home=null` 構造**（永不展開指令側 `~`，因引號 `"~/x"` 在 bash 不展開）；pattern 側才展開 `~`。相對路徑
不對 cwd 解析。canon 對 deny/ask/allow 一致扁平比對，故 argv-specific deny 仍會擋下 exec-only allow 會升級者
...
**bash 語法包裝繞過（out-of-scope，威脅模型明文排除）**：閘②/③ 只擋 agent **常見**的 print-only 假驗證形態，**不防**刻意用 bash 語法包裝來閃避偵測器。已知會落回既有判定（`ask`，或當使用者自設 `Bash(node *)` 等廣域規則時 `allow`）而不觸發硬 deny 的形態包括：pipe producer/consumer 包進 brace group / subshell（`{ echo 'console.log(1)'; } | node`、`( echo … ) | node`）、更深巢狀、以 fd 複製穿插於截斷寫入前（`echo … >&2 > x; node x`）等。另 `recognizeInterpreter` 對**未列入 per-interpreter 安全 allowlist 的分隔旗標**採 fail-safe 保守處理（回 `none`、不 deny），故裸形已擋（`node -e 'console.log("x")'`、`python3 -c 'print("x")'` → deny），但夾帶不常見旗標於 payload 前者（`python3 -u -c '…'`、`node --input-type module -e '…'`）會落回 `ask`（或 `Bash(node *)`/`Bash(python3 *)` → `allow`）。**這類形態永遠不比 pre-feature baseline 差**（本工具在此功能之前本就不擋它們），屬使用者定案接受的 out-of-scope under-deny，**不逐一硬化**（否則無底洞、且與「只擋常見形態、不抗刻意包裝」的核心設計相悖）。若日後要收緊特定形態或擴充旗標 allowlist，走 spec → plan 流程明確擴大威脅模型範圍，而非臨時加碼。
...
**已接受限制（經檢視後擱置收緊方案，非無解）**：
1. **`cd` 一律視為成功**：目標目錄不存在時 bash 會留在原地，後續相對路徑解析到別處，本工具仍照 `cd`
   成功推導 cwd。已知的兩個純詞法收緊方案——只信任 `&&` 之後的 `cd`（`;`/換行後的 `cd` 失敗不影響
   後續指令）、或要求 `cd` 前後兩個 cwd 都通過範圍檢查——因誤殺與改動面被擱置，不是無解。
2. **npm 的設定探索與寫入位置不納入判定**：所有 npm 呼叫都在 dispatch 前沿目錄樹向上讀 `.npmrc`，
   該檔可重導 `cache`/`logs-dir`/`registry`；npm 也會寫入並輪替刪除自己的 debug log。放行
   `npm view`/`ping`/`whoami` 即等於接受這層行為。
...
`sessionTrustedReadRoots`）：`<configDir>/projects/<E>/<session_id>/`（內含 `tool-results/`）與
背景任務輸出目錄（tmp 來源見下）。其中 `E` 取自
hook 傳入 `transcript_path` 之 dirname 的 basename（**權威編碼段、不重算/不猜 Claude 的編碼規則**），`<uid>` 由
`Deno.uid()` 取得（需 `--allow-sys=uid`；取不到 → fail-safe 不產生 POSIX tmp 根）。
...
- **兩道 fail-closed 安全閘**：(a) `basename(transcript_path)` 必須等於 `<session_id>.jsonl`（綁定當前 session）；
  (b) dirname 必須嚴格位於 `<configDir>/projects/` 之下且至少一非空段。另在串接前要求 `session_id` 符合
