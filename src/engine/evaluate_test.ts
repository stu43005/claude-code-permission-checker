...
Deno.test("§1.5 已接受『整鏈 print 洗白繞道』：鏈含真實/no-op 指令即非整鏈 print → 不 deny（刻意、使用者核可）", () => {
  // 裸整鏈 print 仍硬 deny（對照）
  assertEquals(vd("echo fake"), "deny");
  // 鏈中含真實/no-op 指令（含 >/dev/null 丟棄輸出、命令替換包裝）→ 非整鏈 print → 落該指令既有判定。
  // 這是 spec §1.5 明確接受的繞道（維持乾淨結構規則、零誤殺；與 bash -c 同屬刻意接受、需刻意規避），
  // 非 bug——請勿改為 deny。
  assertEquals(vd("pwd; echo fake"), "allow");
  assertEquals(vd("pwd >/dev/null; echo fake"), "allow");
  assertEquals(vd("whoami >/dev/null && echo fake"), "allow");
