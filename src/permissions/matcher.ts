 ...
 * canon 對 deny/ask/allow 三組「一致扁平」比對（皆 canonCmd vs canonPat）。一致性保證關鍵
 * 安全性質：argv-specific 的 canon deny/ask 與 exec-only 的 canon allow 走同一條 canonCmd，
 * 故凡 exec-only allow 會命中的指令，其 path-equivalent 的更具體 deny/ask 也由 canon 命中、
 * 且 deny/ask 先於 allow 評估而擋下（見 §4.5 不變量 11）。raw 分支完整保留。
 */
