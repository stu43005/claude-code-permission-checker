...
for (const s of starts) {
  if (ctx.isDangerousRoot(s)) {
    return deny(`find：遞迴遍歷磁碟根或家目錄根（${s.value}）`);
  }
}
for (const s of starts) {
...
for (const arg of positionals(ctx.argv, valueFlags)) {
  if (isRecursive && ctx.isDangerousRoot(arg)) {
    return deny(`${ctx.name}：遞迴遍歷磁碟根或家目錄根（${arg.value}）`);
  }
...
- `combine_test.ts`：deny 優先（deny+ask+allow → deny；ask+allow → ask；全 allow → allow；
  deny 理由取首個）。
- `scope_test.ts`：`isDangerousRootAbs`（`/`、`C:/`、家目錄、`/usr` 否、子目錄否）；
...
| `src/rules/types.ts` | `RuleVerdict` 加 deny 態、`deny()` ctor、`RuleContext.isDangerousRoot` |
