...
3. **直譯器集合**：`node`、`nodejs`、`python`、`python3`、`deno`、`bun`、`ts-node`。execution-shaping 旗標
   （`--transpile-only`/`--experimental-*`/`--allow-*` 等）**不影響**「會執行該腳本」的判定——帶旗標的 script
   執行仍 deny（使用者 round-17 決定，見 §4.2.1）；僅副作用/預載旗標會使 inline/stdin payload 失去純度。
...
- **value-flags**（資訊性；列出各直譯器帶值旗標）：**本設計刻意不維護精確 arity 表**——§4.2.1 對 script
  執行採「**candidatePaths＝所有靜態非-eval token**，任一等於前驅 P 即 deny」，旗標解析寬鬆也不致誤 deny
  （旗標值幾乎不可能等於剛寫出的全-print 檔路徑，最壞漏 deny）；inline 的 code 直接取 `-e`/`-c` 值。此表
  僅供理解 CLI、不進入辨識邏輯。
...
1. 閘④ **唯一資料來源＝單一 source-order（前序）AST 走訪**（§4.2），**不**讀攤平 `invocations`（攤平後
   ...
   `argv=cmd.suffix`、`assignments=cmd.prefix`、`redirects`＝**該節點自身 redirect ＋ 走訪下降時由外層
   `CompoundList`/`BraceGroup`/`Statement` 累積的繼承 redirect**（與 walk 對複合結構繼承 redirect 一致）、
      ...
      SimpleCommand(node):                               // 任何單一簡單 Command（含 cat/echo/printf/node/python…）
        ...
        if leaf != null and leaf.name in INTERPRETERS and leaf.name not in defs
          ...
          switch recognizeInterpreter(leaf):             // §4.2.1
            ...
            ScriptExec(candidatePaths,lang):             // C（僅 SameShellSeq、緊鄰；旗標無關）
              if prevWriteRef is WRITE(P',content,cwdᵥ,writerName) and writerName not in defs
                 and (∃ P in candidatePaths: pathsEqualCwdAware(P',cwdᵥ, P,cwd))
                 and payloadIsAllStaticPrint(content,lang): found = true
...
`ScriptExec(candidatePaths, lang)` / `Unknown`。

> **旗標策略（回應 round 17、使用者決定）**：使用者明確指出「**execution-shaping 旗標不改變「會執行該
> 腳本」這件事**——重點是會跑一支 print-only 腳本，有沒有 `--transpile-only`/`--experimental-*` 等旗標
> 根本沒差」。故偵測**不再要求裸形式**，而依旗標**是否改變「被執行的內容」**分流：
> - **inline-eval 旗標**（`-e`/`-c`/`--eval`、`deno eval`）→ 取其值為 `code`，模式 `InlineEval`。
> - **stdin 選擇子**（`-`、`deno run -`、`bun run -`、無腳本位置參數）→ `StdinRead`。
> - **副作用/預載旗標**（`-r`/`--require`/`--import`/`--preload`/`-m`/`--env-file`、deno `--allow-*`/`-A`）：
>   它們會**注入額外執行的程式碼**。對 `InlineEval`/`StdinRead`（payload 須「純 print」）→ 這破壞純度 →
>   回 `Unknown`（不 deny）。對 **script 執行** → 不影響「該腳本仍會跑」→ 仍納入 `ScriptExec`。
> - **其餘旗標（良性 mode 旗標、帶值旗標、未知旗標）→ 略過、不致 `Unknown`**。
> - **`ScriptExec.candidatePaths`** ＝ argv 中所有**靜態、非 inline-eval-旗標**的 token（位置參數**與**旗標值
>   皆納入——因 `--loader X`/`--import X`/位置 X 任一都可能是被執行/載入的腳本路徑）。動態 token 不納入。

**為何此放寬安全（不引入誤 deny）**：向量 C 的 deny **只在** `candidatePaths` 中某 token **精確等於**「同鏈
**緊鄰前驅**以全靜態-print 內容寫出的路徑 P」時才成立（§4.5）。即使旗標解析寬鬆（未維護各直譯器帶值
旗標表）導致把某旗標值誤當候選路徑，它也**幾乎不可能**剛好等於「上一條才寫出的全-print 檔路徑」；最壞是
漏 deny（安全）。故**不需**精確旗標 arity 表即可安全涵蓋帶旗標的 script 執行——這正是使用者要的。

- **node / nodejs / bun / ts-node**（lang=js）／**python / python3**（lang=py）：
  - inline：node/bun/ts-node `-e`/`--eval[=]`；python `-c` → `InlineEval(值, lang)`（值動態 → `Unknown`）。
  - stdin：`-` 或無腳本位置參數（且非 inline）→ `StdinRead(lang)`。
  - 否則 → `ScriptExec(candidatePaths, lang)`（candidatePaths＝上述靜態非-eval token；bun `run`、python 皆同）。
  - 副作用/預載旗標：對 inline/stdin → `Unknown`；對 ScriptExec → 不影響（仍納入）。
- **deno**（lang=js）：
  - `deno eval <code>` → `InlineEval(code, js)`（其前若有 `--allow-*`/`--preload` 等副作用旗標 → `Unknown`）。
  - `deno run -` → `StdinRead(js)`。
  - `deno run <…tokens…>` → `ScriptExec(candidatePaths, js)`（candidatePaths＝run 之後所有靜態非-eval token，
    含 `--allow-*`/`--ext X` 等之值與位置參數——任一等於前驅 P 即 deny）。
  - 其他子指令（`deno test`/`deno task`/…）→ `Unknown`。
  - 註：`-p`/`--print`（運算式吐值、非 `printfn(...)` 敘述）→ 述詞天生不命中 → 自然不 deny（§1.4.1）。
...
- command 為 `Pipeline`/`Subshell`/`If`/`For`/`While`/`Case`/`Function` 等複合節點 → 回 `null`（該節點
  本身非單一簡單 `Command`；走訪仍**下降**其內以收集函式定義與對其內直譯器跑向量 A/B，但不延續 C/D 相鄰）。
...
1. **循序序列（sequential sequence）＝「當前 shell、無條件循序」的容器**：`Script.commands`、
   `CompoundList`、`BraceGroup`、以 `&&`/`;` 連接的 `AndOr` 成員。向量 C 的「緊鄰前驅」只在**同一個
   循序序列內的相鄰 sibling** 成立。`Subshell`/`Pipeline`/`If`/`For`/`While`/`Case`/`Function`、命令替換、
   **以 `&` 背景化/async 的 statement、coproc**、以及以 `||` 連接的成員一律**不算循序序列、不延續 `prevWrite`**
   （走訪仍下降其內跑向量 A/B 與收集函式定義，但 C/D 相鄰不跨此邊界），以免在「執行順序/可達性無保證」處
   誤判。**WRITE 與 EXEC 兩者皆不得為背景/async statement**（回應 round 4 finding 2：`cat > x & node x`
...
2. **緊鄰前驅比對**（cwd-aware，回應 round 2 finding 1；旗標無關，回應 round 17）：對循序序列上每個節點，
   `leafOf` 取其 leaf。若某 statement 為 **`ScriptExec(candidatePaths, lang, cwdₑ)`**（`leafOf` 名 ∈
   INTERPRETERS、非 inline/stdin），檢查其**緊鄰前一條** statement 是否為 **`WRITE(P', content, cwdᵥ)`**
   （靜態寫腳本檔，見下）。比對使用**各自 statement 的 cwd 快照**：**僅當 `cwdᵥ` 與 `cwdₑ` 皆為 `known`
   且 candidatePaths 中**某** P 滿足 `normalizeAbsolute(P', cwdᵥ) === normalizeAbsolute(P, cwdₑ)`** 才算
   「執行了該寫出檔」；任一快照 `unknown` 或無此 P → **跳過、不 deny**。命中且
   `payloadIsAllStaticPrint(content, lang)` → **deny**。否則不 deny。`ts-node --transpile-only x.ts`、
   `node --experimental-* x.mjs`、`deno run -A x.ts` 等帶旗標形式 → x 仍在 candidatePaths → 命中即 **deny**。
...
- script 執行旗標無關（回應 round 17）：`echo 'console.log("x")' > verify.ts; ts-node --transpile-only verify.ts`
  ...
  → **deny**（x 仍在 candidatePaths、命中前驅 P）；對照路徑不符 `… > a.mjs; node b.mjs` → 不 deny。
