...
- `-p`/`--print <expr>`（node/bun/ts-node、`deno eval -p`）吐出 `<expr>` 之值。**回應 round 32 finding 2**：
  以 `printExprIsStaticString(expr)` 判——`<expr>` 為**靜態字串字面量、或僅字串字面量以 `+` 串接**（無識別字
  /數字算術/呼叫/模板表示式/反引號含 `${}`）→ 印出寫死字串＝print 偽裝 → **deny**（`node -p '"fake
  verification passed"'`、`node -p '"a"+"b"'`）。否則（`1+1` 算術、`os.cpus()` 呼叫、變數）→ 真實運算 →
  **不 deny**。此 predicate 與 §4.1 同為 fail-safe（任何非純字串字面量 → 不 deny）。
...
- `-p`/`--print` 吐值：僅在 `<expr>` 為**純字串字面量/字串字面量串接**時 deny（印寫死字串，§4.2.1 的
  `printExprIsStaticString`）；含算術/呼叫/變數的運算式 → 不 deny（YAGNI 不解析任意運算式語意）。
...
- **全域硬上限（DoS 後盾，回應 round 32 finding 3；調和 round 7 finding 2）**：以**高門檻**設全域上限，正常
  指令不會觸及——`MAX_TOTAL_BYTES`（建議 **2 MiB**，≥ 多數 OS 的 `ARG_MAX`）、`MAX_CANDIDATES`（候選 payload
  數，建議 **4096**）、`MAX_MEMO_ENTRIES`（建議 **4096**）。**任一全域上限超出 → gate ④ 立即整體 `false`
  （under-deny、不 deny）**。此後盾只在「**單一指令異常巨大/候選異常多**」的病態輸入下觸發（同步 hook 必須
  有界、永遠 `exit 0`）；其代價是該病態指令整體 under-deny（**安全方向**）。**與 round 7 的差異**：round 7
  反對的是「**低門檻**全域 byte 預算」（會讓正常大 payload 提前耗盡、隱藏後續小相符）；此處門檻設在 OS arg
  上限級別，正常工作流不會命中，僅作 DoS 後盾——這是已記錄、刻意接受的 under-deny 尾段（病態輸入安全退回）。
  ...
  visit(node, cwd, defs, prevWriteRef):
    switch node:
      ...
      SimpleCommand(node):                               // 任何單一簡單 Command（含 cat/echo/printf/node/python…）
        ...
        exec = (leaf != null) ? resolveExec(leaf) : null // §4.2.0：透視 command/env/timeout/nice/nohup wrapper；回傳 .viaWrapper
        if exec != null and exec.name in INTERPRETERS
           and leaf.name not in defs                     // 外層（shell 最先解析的）名未被遮蔽
           and (exec.viaWrapper or exec.name not in defs)  // 內層名遮蔽：**僅 bare（非 wrapper）時**才算（回應 round 20/29）：
                                                           //   wrapper（command/env/timeout/…）以 execvp 跑外部 binary、command 明確繞過函式 → 內層 node 函式不會執行 → 內層遮蔽無關；
                                                           //   bare node 則內外同名、檢查 node 遮蔽。
           and assignmentsIdentitySafe(leaf.assignments):  // 賦值前綴：僅良性（非 SENSITIVE_ENV、靜態）才 deny-eligible（round 14/31）
                                                            //   X=1 node -e 'fake' → deny；PATH=…/LD_PRELOAD=…/NODE_OPTIONS=… node → Unknown
...
> 指令解析）、且**「有效 fd1（stdout）目標」為靜態截斷重導向** `>`/`>|` 到靜態路徑 P（**最後者勝**，回應
> round 30 finding 2：依 Bash 重導向順序，影響 fd1 的重導向中**最後一個**才是內容去處）、且內容可靜態還原
> （§4.5 步驟 3）。**例**：`cat > x > sink <<EOF…EOF` → 有效 fd1＝`sink`（最後）、`x` 只被截斷成空 → P=`x`
> **非 content-write** → 不回 WRITE（避免誤 deny `… > x > sink …; node x`）；`cat > x 1>&2 <<EOF…EOF` → 有效
> fd1＝stderr（fd 複製）→ 非寫檔 P。否則回 `null`。任何**非簡單指令**或無此有效寫入 → `prevWrite` 重置為
> `null`。**向量 D 的生產者/消費者**同理：任一段帶賦值前綴 → 身分不確定 → 跳過、不 deny。
...
- `env [NAME=VAL…] CMD…`：**只在 `NAME=VAL` 全為「身分/注入無關」的良性變數時透視**（回應 round 31 finding 1：
  `env X=1 node -e 'fake'` 中 `X=1` 不改 node 身分 → 應 deny）。**敏感變數集合 `SENSITIVE_ENV`**（任一出現或
  變數名/值動態、或 `-i`/`--ignore-environment`/`-u NAME` → `Unknown`）：`PATH`/`PATHEXT`（改指令解析＝身分）、
  `LD_PRELOAD`/`LD_LIBRARY_PATH`/`LD_*`/`DYLD_*`、`NODE_OPTIONS`/`NODE_PATH`、`PYTHONPATH`/`PYTHONSTARTUP`/
  `PYTHONHOME`、`BASH_ENV`/`ENV`（注入碼/改解析）。否則（僅良性變數）→ 透視成 `CMD…`、deny-eligible。
  此 `SENSITIVE_ENV` 為**可擴充清單**；其中 `PATH`/`PATHEXT` 即唯一「改身分」者（已含），故漏列其他純注入
  變數最多讓「含注入的 static-print 偽裝」照樣 deny（payload 仍是 print、合理），**不致改身分而誤 deny**。
...
>
> **wrapper × 函式遮蔽（最終、技術正確；解決 round 20↔21↔29 振盪）**：遮蔽檢查施於 **shell 最先解析的名**：
> - **bare 直譯器**（無 wrapper）：檢查直譯器名（= 最外層名）是否被同 scope 函式遮蔽；`node(){:;}; node -e '…'`
>   → node ∈ defs → 跳過、落閘③ ask（函式實際執行）。
> - **wrapper 透視**（`command`/`env`/`timeout`/`nice`/`nohup`/`setsid`/`stdbuf`）：**只檢查外層 wrapper 名**；
>   **內層直譯器名的函式遮蔽無關**——`timeout`/`env`/`nice`/`nohup` 以 **`execvp`** 跑**外部 binary**、`command`
>   **明確繞過函式**，其 exec 的 `node` 必是 binary 而非 shell 函式（匯出函式 `BASH_FUNC_*` **只對子 bash 可見**、
>   對 `execvp` 的非-bash 子行程無效）。故 `node(){:;}; timeout 5 node -e 'fake'`（含 `export -f node`）→ real
>   timeout exec node **binary** → **deny**（回應 round 20/29；內層 node 函式不會執行）。
> - 外層 wrapper 名被遮蔽（`timeout(){:;}; timeout 5 node …`）→ 身分不確定 → 跳過、ask。
> e2e 須涵蓋：bare `node(){}; node -e` → ask；`node(){}; command|env|timeout|nohup node -e 'fake'`（含
> `export -f node`）→ **deny**；`timeout(){}; timeout node …` → ask。
...
>   `--files`；python `-O`/`-B`/`-u`/`-v`/`-q`/`-I`/`-s`/`-E`/`-i`；deno `-A`/`--allow-all`/`--no-check`/
>   `--quiet`/**裸 `--allow-read`/`--allow-write`/`--allow-net`/`--allow-env`/`--allow-run`/`--allow-sys`/
>   `--allow-ffi`/`--deny-*`**（回應 round 27 structural：deno 權限旗標裸用是 no-value；其 `=value` 形如
>   `--allow-read=path` 屬 `=` 形自含值、1 token）。（`--experimental-default-type=module` 亦屬 `=` 形。）
> - **覆蓋宣稱＝flagless ＋ enumerated-known-flags（明確、回應 round 22/27 finding；使用者 converge 決定）**：
>   本設計**不宣稱 flag-agnostic 全覆蓋**。**硬 deny 的 script-執行覆蓋僅保證**：(a) 裸形式 `node x.mjs`、
>   (b) 旗標**全在上方「已知無值旗標集合」或 `=` 形或已知帶值旗標**之列者。**未列入的未知 no-value 旗標**
>   （含 CLI 版本演進新增者）會被當帶值、吃掉 entrypoint → **明確的 under-deny（安全；可被 `Bash(node *)`
>   升級，屬既有 settings 風險，非本功能新增）**。此即「依小而可能過時的旗標清單、而非『該指令是否真是
>   static-print script 執行』」之**已知版本偏移邊界**——與跨呼叫拆分（§5.1 item 1）同屬使用者接受、刻意
>   記錄的 under-deny。**無值集合刻意維護為可擴充清單**（新增常見旗標即提升覆蓋）；其誤漏方向**恆為
>   under-deny、絕不誤 deny**。（不採「未知→當無值」是因那會把帶值旗標之值誤當 entrypoint → 誤 deny、違反
>   zero-false-deny。）
...
>   **option boundary（修正 round 24 advisory；回應 round 30 finding 1）**：`-e`/`-c` 的值之後的 token 是
>   **程式 argv、非直譯器選項**（node `-e 'code'`、python `-c 'code'` 後皆終結選項）。故**只掃 inline-eval
>   旗標之前**的旗標判預載/副作用：`node -r x -e '…'`（-r 在前）→ `Unknown`；**`node -e '…' -r x`、
>   `python -c 'print("fake")' -m anything`（旗標在 code 之後＝程式 argv）→ 不破壞純度 → 仍 deny**。
>   對 **script 執行**：預載旗標之值被當帶值 token 吃掉（非 entrypoint）→ 不影響、亦不誤 deny preload 檔。
> - 動態 token（entrypoint 或 inline 值動態）→ `Unknown`。**`-p`/`--print <expr>` → `printExprIsStaticString`：
>   純字串字面量/字串串接 → deny（印寫死字串）；否則不 deny（§1.4.1，回應 round 32 finding 2）。**
...
3. **`WRITE(P, content)` 定義**：statement 的 leaf 之**有效 fd1（stdout）目標**為**截斷型重導向 `>`/`>|`**
   到**靜態路徑** P（**最後者勝**：影響 fd1 的重導向取最後一個，回應 round 30 finding 2——`cat > x > sink
   <<EOF…EOF` 有效 fd1＝sink、x 只截斷成空 → 非 P；`cat > x 1>&2 <<EOF` 有效 fd1＝fd 複製 → 非寫檔），
   且內容可由該 leaf 靜態還原：`cat`/`tac` 搭靜態 heredoc/here-string（`isHeredocPrintEligible`）→
...
8. **指令身分不確定（回應 round 14/19/31）**：(a) 賦值前綴/`env NAME=VAL` 帶 **`SENSITIVE_ENV`**（`PATH`/
   `LD_*`/`DYLD_*`/`NODE_OPTIONS`/`NODE_PATH`/`PYTHONPATH`/`PYTHONSTARTUP`/`BASH_ENV` 等，§4.2.0）→ 身分/注入
   不確定 → **不 deny**；**良性變數**（`X=1`）→ 不影響身分 → 仍 deny-eligible（`env X=1 node -e 'fake'`、
   `X=1 node -e 'fake'` → **deny**，回應 round 31 finding 1）。(b) **保留執行語意的 exec wrapper**
   （`command`/`nohup`/`nice`/`timeout`/`env`(良性環境)/`setsid`/`stdbuf`）→ §4.2.0 **透視**後仍走 gate ④ →
   `timeout 5 node -e 'fake'`、`command node -e 'fake'` 等 **deny**（回應 round 19，閉合此繞道）。**殘留
   under-deny**：env 帶 `SENSITIVE_ENV`/`-i`、未知 wrapper、wrapper 帶動態參數 → `Unknown`、不 deny。
9. **旗標清單版本偏移（回應 round 27；使用者 converge 接受）**：script-執行覆蓋僅及「flagless ＋ 列舉之
   已知旗標」；未列入無值集合的**未知 no-value 旗標**（CLI 演進新增）→ 吃掉 entrypoint → under-deny（可被
   `Bash(node *)` 升級）。與跨呼叫拆分同屬已記錄、刻意接受的 under-deny 邊界（無值集合可隨需擴充，誤漏方向
   恆為 under-deny、絕不誤 deny）。§4.2.1 詳述。
10. **同呼叫函式體（回應 round 22/31 finding 2；刻意接受）**：`f(){ node -e 'fake'; }; f`（同呼叫定義並呼叫）
   → gate ④ **不解析函式體**（避免 over-deny 未呼叫的 dead body、且不做同呼叫函式內聯之 YAGNI）→ 落既有
   **閘③ 函式遮蔽 → 不可升級 ask**。即 same-call 可執行函式體 payload 的政策結果為**不可升級 ask（非 deny）**，
   仍非放行；屬已記錄、刻意接受的取捨（被呼叫名 ∈ whole-script fnNames → 閘③ 必擋）。
...
- 向量 C（含順序感知，回應 round 1 design-soundness）：
  ...
  - **append / 背景（回應 round 4 findings）**：`echo 'console.log("x")' >> real.js; node real.js`（append `>>`）
    ...
    → **不 deny**（背景不保證寫入先完成）。
  - **有效 fd1 最後者勝（回應 round 30 finding 2，不得誤 deny）**：`cat > x.mjs > sink <<'EOF'…EOF; node x.mjs`
    （有效 fd1=sink、x.mjs 只截斷成空）→ **不 deny**；`cat > x.mjs 1>&2 <<'EOF'…EOF; node x.mjs`（fd 複製到
    stderr）→ **不 deny**；對照 `cat > x.mjs <<'EOF'…EOF; node x.mjs`（有效 fd1=x.mjs）→ deny。
...
- `-p`/`--print` 字串字面量（回應 round 32 finding 2）：`node -p '"fake verification passed"'`、`node -p '"a"+"b"'`、
  `bun -p '"fake"'` → **deny**（印寫死字串）；`node -p '1+1'`、`node -p 'os.cpus()'`、`node -p 'process.version'`
  → 不 deny（真實運算/變數）。
- 全域上限後盾（回應 round 32 finding 3）：候選 payload 數 > 4096 或總位元組 > 2 MiB → gate ④ 整體 `false`
  （under-deny、安全）；正常指令遠低於門檻、不觸及。
...
- 指令身分（回應 round 14/19/20）：`PATH=$PWD/bin:$PATH node -e 'console.log("x")'`（賦值前綴）→ 不 deny；
  ...
  **deny**（§4.2.0）**；`env X=1 node -e …`（良性變數）→ **deny**；`env PATH=/x node -e …`/`env -i node -e …`
  （SENSITIVE_ENV/清環境）→ 不 deny（回應 round 31/32）。**wrapper × 遮蔽（最終、技術正確，round 20↔21↔29）**：
  **`node(){:;}; timeout 5 node -e 'fake'`、`node(){:;}; export -f node; command node -e 'fake'`（內層 node
  被遮蔽、但 wrapper exec binary）→ **deny**（內層函式不執行）**；bare `node(){:;}; node -e 'fake'`（無 wrapper）
  → ask；外層 `timeout(){:;}; timeout 5 node …`（wrapper 名被遮蔽）→ ask。
- 直譯器辨識（inline 純度、option boundary，回應 round 30 finding 1）：`node -r x -e 'console.log(1)'`（-r 在
  eval 旗標**之前**）→ Unknown、不 deny；**`node -e 'console.log("fake")' -r x`、`python -c 'print("fake")' -m
  anything`（旗標在 code **之後**＝程式 argv）→ **deny**（不破壞純度）**；`deno eval --allow-read
  'console.log("x")'`（inline + 副作用）→ Unknown、不 deny；裸/帶良性旗標
...
- exec wrapper 透視（回應 round 19/21）：`timeout 5 node -e 'console.log("fake")'`、`command node -e '…'`、
  ...
  `Bash(timeout *)` 時 `timeout 5 node -e '…'` 仍 **deny**（不經 settingsAllows）；`env PATH=/x node -e '…'`
  （SENSITIVE_ENV）、`foowrap node -e '…'`（未知 wrapper）→ 不 deny（under-deny）。
- script 執行已知旗標（回應 round 17/27）：`echo 'console.log("x")' > verify.ts; ts-node --transpile-only verify.ts`
  → **deny**；`… > x.mjs; node --experimental-default-type=module x.mjs` → **deny**；`… > x.ts; deno run -A x.ts`、
  **`… > x.ts; deno run --allow-read x.ts`、`deno run --allow-read=./d x.ts`（裸/=形 allow 旗標）** → **deny**
  （entrypoint=x）；對照路徑不符 `… > a.mjs; node b.mjs` → 不 deny。
- **版本偏移邊界（回應 round 27，明確 under-deny）**：`… > x.mjs; node --some-future-no-value-flag x.mjs`
  （未列入無值集合的 no-value 旗標）→ 吃掉 x.mjs → 無 entrypoint → **不 deny**（安全、可被 `Bash(node *)`
  升級；已記錄之版本偏移邊界，非 bug）。
...
- 函式體不下降（回應 round 22/31 finding 2）：`f(){ node -e 'console.log("fake")'; }`（只定義、未呼叫）→ **不 deny**
  （body 不執行）；`f(){ node -e 'console.log("fake")'; }; f`（呼叫）→ gate ④ 不解析函式體 → 落閘③（f 被
  呼叫且 ∈ fnNames）→ **不可升級 ask**（刻意接受，§5.1 item 10）；對照 `( node -e 'fake' )`（subshell body 會執行）→ deny。
- env/賦值良性 vs 敏感（回應 round 31 finding 1）：`env X=1 node -e 'console.log("fake")'`、`X=1 node -e 'fake'`
  （良性變數）→ **deny**；`env PATH=/x node -e 'fake'`、`NODE_OPTIONS=--require=./p node -e 'fake'`、
  `LD_PRELOAD=./e.so node -e 'fake'`、`env -i node -e 'fake'`（SENSITIVE_ENV/清環境）→ **不 deny**。
