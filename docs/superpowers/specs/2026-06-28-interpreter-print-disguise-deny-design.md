...
interpreterPrintSprayDeny(script, initialCwd):
  ...
  visit(node, cwd, defs, prevWriteRef):
    switch node:
      ...
      AndOr(legs, ops):            // `&&`/`||` 鏈（成功閘控，回應 round 23）
        local c = cwd
        for i, leg in legs:
          // 僅「第一個 leg 是 WRITE、第二個 leg 是 EXEC」這對才認 prevWrite；其餘 leg 之間 reset（reachability 有條件）
          local prev = (i == 1 and legs[0] 為 simple WRITE and ops[0] == "&&") ? WRITEof(legs[0]) : null
          c = visit(leg, c, defs, &prev)
...
1. **循序序列與 `&&` 成功閘控（回應 round 23 finding：`&&` 帶成功閘控、不可等同 `;`）**：
   ...
   - **`&&`-`AndOr`**：第 i leg 須前 0..i-1 leg 皆成功才執行，**reachability 有條件**。故僅認**恰為
     `WRITE && EXEC` 的配對**——即 WRITE 是其 `&&` 鏈的**第一個 leg**（前面**無**其他 `&&` leg）、EXEC 緊接其後。
     `cat > x <<EOF…EOF && node x` → deny（WRITE 首 leg、成功才執行 node，airtight）；
     **`false && cat > x <<EOF…EOF && node x` → WRITE 非首 leg（前有 `false`）→ 不 deny**（修 round 23：
     `false` 短路、整鏈不執行，避免誤 deny inert 鏈）。
...
- 向量 C（含順序感知，回應 round 1 design-soundness）：
  ...
  - **`&&` 成功閘控（回應 round 23）**：`cat > x.mjs <<'EOF'…EOF && node x.mjs`（WRITE 為 `&&` 鏈首 leg）→
    **deny**（airtight）；**`false && cat > x.mjs <<'EOF'…EOF && node x.mjs`、`x && cat > x.mjs <<'EOF'…EOF &&
    node x.mjs`（WRITE 非首 leg、前有 `&&` leg）→ **不 deny**（短路/條件到達，避免誤 deny inert 鏈）**；
    `true || cat > x.mjs <<'EOF'…EOF && node x.mjs` → 不 deny；`false && cat > x.mjs <<'EOF'…EOF; node x.mjs`
    （`&&` 鏈後接 `;`）→ node x 不 deny（prevWrite 不跨 `&&` 邊界外帶）。
