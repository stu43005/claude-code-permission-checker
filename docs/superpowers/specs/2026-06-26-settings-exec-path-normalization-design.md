...
4. **`~` 或 `~/` 開頭**：
   ...
   - 否則 `~` → `home`、`~/x` → `home` + `/x`，**並對展開後路徑重套規則 2（前導 `//` UNC）與規則 3（`..` 段）fail-closed**：若 `home` 本身為 UNC（`//server/...`）或含 `..` 段，展開結果會把 UNC 根詞法折疊成本機絕對路徑（誤跨命名空間）、或在 symlink 下失真 → **拒絕展開、原樣返回 token**。通過則此時已是絕對路徑 → 落規則 6 的絕對分支。
   - （規則 2/3 在展開前只看原 token，故 `~` 展開必須在展開**後**再次套用同樣的 UNC/`..` 守門，否則 UNC home 會繞過規則 2。）
...
4. **UNC / 前導 `//` fail-closed（含 `~` 展開後重檢）**：前導雙斜線（potential UNC）的 token 一律**原樣保留、不折疊、不 toPosix**。本層**不對 UNC 做語義正規化**（不解析 UNC 根、不折疊前導 `//`）；保留字面使 UNC token **仍可與字面相同的 pattern 命中**，但不會因折疊前導 `//` 而與本機絕對路徑規則（`/server/share/...`）誤撞。結果至多配不到 → 維持 `ask`，**絕不誤放**。**`~` 展開後必須重檢**：規則 2 在展開前只看原 token，若 `home` 本身為 UNC（`//server/share/...`），`~/x` 展開成 `//server/share/.../x` 後若不重檢，會被 `normalizeAbsolute` 折疊成本機 `/server/share/.../x` 而把 `Bash(~/x *)` 誤放行本機路徑。故規則 4 在展開後再次套用前導 `//`（與 `..` 段）守門、不安全則拒絕展開（§4.2 規則 4）。
...
6. **執行檔路徑含空白：pattern 側不享正規化加成，但安全（union 保護）**：指令側 `inv.name` 是 parser 的單一 token，含空白也完整、正規化涵蓋全路徑、無切割問題。pattern 側則以第一個空白切 head，若 pattern 的執行檔路徑含空白（引號/跳脫），head-split 只會正規化空白前段。因比對採 union（§4.4 / §4.5 不變量 1）仍保留 `(rawCmd vs rawPat)`，**現行字面行為完整保留**：含空白的 deny/ask/allow 規則一律照舊生效，最壞只是該 pattern 拿不到 `//`/`~`/`.` 正規化加成（回退 raw 字面比對）。**不**會破壞規則、**不**會弱化 deny。**deny 優先安全網（相對官方 baseline 不弱化的證明）**：正規化只在**指令含 `//`/`.`** 時才可能新增 allow 命中；而當指令含 `//` 時，路徑等價的 deny 必被擋下——`//`-deny 由 raw 分支命中（指令亦含 `//`）、`/`-deny 由 canon 分支命中（指令折疊後等於 `/`-deny）。故任何「正規化新增的 allow」對應的指令都先被 deny 攔截。至於「`//`-deny × `/`-指令」此種 head-split 未折疊空白後 `//` 的情形，**官方字面語義本就不命中**（`//` ≠ `/`），我們以 raw 分支原樣重現官方結果，**非**我們新增的弱化。
