...
- G5：**兩道 fail-closed 安全閘**（任一不過 → 不放寬，回 `[]`）：
  - **(a) session 綁定**：`basename(normalizeAbsolute(transcript_path))` 必須等於 `<session_id>.jsonl`。
    ...
    **且 `session_id` 必須是安全單一路徑段**：符合 `^[A-Za-z0-9_-]+$`（UUID 形如
    `115826ef-e830-461f-8101-edac56694d2b` 即符合），藉此**拒絕 `.`/`..`/含 `/`/`\`/含 `.` 等惡意值**，
    避免串入路徑後被 `normalizeAbsolute` 收斂逃出 session 子樹（如 `sid="."`→`dir`、`sid=".."`→`projects` 根）。
...
3. **`sid` 安全段檢查**：`sid` 不符 `^[A-Za-z0-9_-]+$`（即含 `.`/`/`/`\`/空白/為空 等）→ `[]`。
   此步**先於**任何把 `sid` 串入路徑的動作，封住 `sid="."`/`".."`/`"a/b"` 等逃逸。
4. `abs = normalizeAbsolute(t)`。**G5(a)**：`posixBasename(abs)` 不等於 `sid + ".jsonl"` → `[]`。
5. `dir = posixDirname(abs)`；`dir === null` → `[]`。
6. **G5(b)**：`projectsRoot = normalizeAbsolute(<home>/.claude/projects)`；
   `isWithin(projectsRoot, dir) === false` 或 `dir === projectsRoot` → `[]`。
7. `E = posixBasename(dir)`。`roots = [ normalizeAbsolute(dir + "/" + sid) ]`（~/.claude session 子目錄）。
8. 若 `uid !== null`：`bases = includePrivateTmp ? ["/tmp", "/private/tmp"] : ["/tmp"]`；
   對每個 `b` 推入 `normalizeAbsolute(b + "/claude-" + uid + "/" + E + "/" + sid)`（/tmp session 子目錄）。
9. **Post-construction 不變量**（defense-in-depth）：對每個 root 斷言 `posixBasename(root) === sid`
   （~/.claude root 另須 `posixDirname(root) === dir`）；任一不符 → 回 `[]`。step 3 已保證此恆成立，
   此步僅防未來建構邏輯變更回歸。
10. 回傳 `roots`（1～3 個根，皆 `.../<sid>/`）。

異常案例（皆 fail-safe）：`sid="."`/`".."`/含分隔符（step 3 拒）、`~/.ssh/<sid>.jsonl`（不在 projects 下 →
G5(b) 拒）、basename 與 `session_id` 不符（G5(a) 拒）、`<home>/.claude/projects/<sid>.jsonl`（dir == projectsRoot
→ G5(b) 拒）；錯專案 transcript `<other>/<sid>.jsonl` 即便過閘，其 `<other>/<sid>/` 也不含當前 session 真實資料
（session 子樹由唯一 sid 界定）。
...
- `src/claude_dir_test.ts`（新）：
  ...
  - **`sid` 安全段負向（path-escape）**：`session_id` 為 `"."`（transcript `…/<E>/.jsonl`）、`".."`
    （`…/<E>/...jsonl`）、`"a/b"`、`"a.b"`、含 `\` 或空白 → `[]`（不得收斂出 `<E>` 或 `projects` 根）。
    並斷言合法 UUID（如 `115826ef-e830-461f-8101-edac56694d2b`）通過、且每個 root 的 basename 恰為該 sid。
