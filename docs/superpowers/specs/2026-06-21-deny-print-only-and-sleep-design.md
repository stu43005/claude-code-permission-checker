...
main.ts → evaluate(command, root, initialCwd, rules, home, trustedReadRoots)
  └─ parse → walk → invocations[]   （walk 新增：列舉 redirect.body 內的 $() → 內層指令成獨立 invocation）
       ...
       │  （先算 fnNames = definedFunctionNames(script)、isShadowed；遮蔽名不參與下方 deny 閘）
       ├─ 閘①(deny)：some(name === "sleep" && !shadowed) → deny(pollingDenyReason())   （classify 前短路）
...
// 閘 ①（deny）：鏈中出現「**未被遮蔽**」的裸 sleep（含命令替換內層 sleep）
if (invocations.some((inv) => inv.name === "sleep" && !isShadowed(inv))) {
  return { verdict: "deny", reason: pollingDenyReason() };
}
// 閘 ②（deny）：整鏈皆 print 形態——僅當**無任何名稱被遮蔽**時才信任（遮蔽使 name 不可信）
if (!anyShadowed && isAllPrintOnly(invocations)) {
  return { verdict: "deny", reason: printOnlyDenyReason() };
}
// 閘 ③（ask）：存在被遮蔽的呼叫名 → name 分析不可信 → 交人工確認
if (anyShadowed) {
...
- **三閘皆在 `classify` 之前返回 → 不經中央前置規則、不經 `settingsAllows`**：閘①②為 deny（真正硬性）、
  閘③為 ask（不可被 `permissions.allow` 升級）。
- **遮蔽優先序的關鍵**：閘①②**只對未被遮蔽的可信名稱**生效——`echo(){ grep x f; }; echo` 因 echo 被
  遮蔽，閘②的 `!anyShadowed` 為 false 而**跳過**，落閘③ → ask（而非誤判 print-only deny）；對照真實
  `echo "fake"`（無函式定義）→ anyShadowed false → 閘② deny。混合情形 `foo(){:;}; foo; sleep 5` →
  sleep 未被遮蔽 → 閘① deny（真實 sleep 仍硬擋）。
...
**與 deny 閘的順序（重要——遮蔽名不參與 name-based deny）**：遮蔽使 name 不可信，故 §4.2 的 deny 閘
**只對未被遮蔽的可信名稱**生效：
- 閘①（sleep deny）條件含 `&& !isShadowed(inv)`：`echo(){ grep x f; }; echo`、`date(){ sleep 5; }; date`
  不會誤觸發；真實 `sleep 5`（未遮蔽，即使同腳本另定義無關函式如 `foo(){:;}; foo; sleep 5`）仍 deny。
- 閘②（print-only deny）以 `!anyShadowed` 為前提：`echo(){ grep needle README.md; }; echo` 因 echo 被
  遮蔽 → **跳過**閘② → 落閘③ ask（**不**誤判 print-only deny、理由不誤導）。
- 閘③（ask）在閘①②之後，承接所有「含遮蔽名」的情形。`sleep(){ :; }; sleep 5`（把 sleep 覆寫成 no-op）
  → 閘① `!isShadowed` 為 false 跳過 → 閘② anyShadowed 跳過 → 閘③ ask（而非把 no-op 誤 deny）。
...
- `src/engine/walk_test.ts` / `evaluate_test.ts`（§4.9 函式遮蔽）：`definedFunctionNames` 對
  ...
  （ls 未被遮蔽，照常 allow）；`echo(){ grep needle README.md; }; echo` → **ask**（被遮蔽 → 跳過閘②
  print-only deny，非誤硬 deny）；`sleep(){ :; }; sleep 5`（sleep 被遮蔽成 no-op）→ **ask**（閘①
  `!isShadowed` 跳過、閘③ ask）；對照 `foo(){ :; }; foo; sleep 5`（sleep 未被遮蔽）→ **deny**（閘①）。
