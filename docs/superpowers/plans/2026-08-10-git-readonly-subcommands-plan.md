...
**Architecture:** 全部變更集中在單一規則檔 `src/rules/commands/git.ts`。`READ_SUBCOMMANDS` 集合直接擴充；`parseSub` 額外回傳 `restWords: Word[]`（子指令之後的原始 Word），供新的 `scanRestArgs` 走訪函式使用——該函式在 `--` 處停止掃描，對動態 token 回 ask，對 `-O` 的兩種寫法做 `resolvePathValue` 範圍檢查。`--help` 封堵需要兩處改動：從 `SAFE_VALUELESS_GLOBAL` 移除（涵蓋全域位置），加 rest 層檢查（涵蓋子指令之後）。
...
| 檔案 | 動作 | 責任 |
|---|---|---|
| `src/rules/commands/git.ts` | Modify | git 指令規則的唯一實作點。本次全部行為變更都在此檔：子指令集合、`parseSub` 回傳值、新的 `scanRestArgs` 掃描函式、`evaluate` 的檢查順序。 |
| `src/rules/commands/git_test.ts` | Modify | 對應的單元測試。既有測試**不需修改**（已逐一檢視，見下），只追加新測試。 |
| `CLAUDE.md` | Modify | 專案文件中描述 git 規則攻擊面的段落需同步。 |
...
Run: `deno test --allow-env --filter "newly added read-only subcommands" src/rules/commands/git_test.ts`

Expected: FAIL — 斷言錯誤 `Values are not equal: expected "allow", actual "ask"`（`merge-base` 尚未列入 allowlist）。

`excluded subcommands still ask` 這條在此時應已 PASS（那些子指令本來就落 `default: ask`）；它是防止 Task 1 誤加的護欄。
...
- Modify: `src/rules/commands/git.ts:178-181`（`evaluate` 的 rest 層檢查區）
- Test: `src/rules/commands/git_test.ts`
...
Run: `deno test --allow-env --filter "--help paths ask" src/rules/commands/git_test.ts`

Expected: FAIL — `git --help log` 與 `git log --help` 目前皆回 `allow`。

- [ ] **Step 3: 移除全域安全清單中的 `--help`**

在 `src/rules/commands/git.ts` 的 `SAFE_VALUELESS_GLOBAL` 中，刪除 `"--help",` 這一行，並在該常數的 doc comment 補充理由。改動後該常數末段為：
...
插入後該區塊順序為：`--ext-diff` → `--help` → `--output` → `git grep -O`。
...
特別確認 `git safe valueless global options allow with read subcommands` 這條既有測試仍 PASS——它不含 `--help`，不受影響。
...
### Task 3: `parseSub` 回傳 `restWords` + 動態 token 收緊

`-O` 檢查建立在「能靜態看到所有引數」之上。若放行動態 token，`git diff $ARGS HEAD` 在 runtime 展開為 `git diff -O/etc/passwd HEAD` 時仍以展開前的形狀分類，新檢查會被單一變數整個繞過。靜態分析無從區分 `$FOO` 展開成 branch 名或旗標，故 fail-safe 處理是 ask。
...
Run: `deno test --allow-env --filter "dynamic token after subcommand" src/rules/commands/git_test.ts`

Expected: FAIL — `git log $ref` 目前回 `allow`（`parseSub` 以哨符 `" "` 代表動態值、不 ask）。

`static tokens that look exotic are still static` 此時應已 PASS，作為防止過度收緊的護欄。
...
Expected: 無錯誤（`_ctx` 前綴底線避開未使用參數警告；Task 4 會用到它）。
...
Deno.test("-O orderfile attached form: in-project allows, outside asks", () => {
  ...
  assertEquals(v("git diff-tree -O../outside.txt HEAD"), "ask");
});
...
Run: `deno test --allow-env --filter "-O orderfile" src/rules/commands/git_test.ts`

Expected: FAIL — `git diff -O/etc/passwd HEAD` 目前回 `allow`。

- [ ] **Step 3: 在 `scanRestArgs` 中加入 `-O` 兩種形式的處理**

把 Task 3 建立的 `scanRestArgs` 整個替換為（注意參數 `_ctx` 改名為 `ctx`，並更新 doc comment）：
...
This also closes the same gap on the pre-existing diff/log/show entries, not
just the newly added subcommands.
...
**Files:**
- Modify: `CLAUDE.md`（「安全誤放」段落中描述 git 攻擊面的項目）

- [ ] **Step 1: 更新 CLAUDE.md**

在 `CLAUDE.md` 的「⚠️ 不要再犯的問題 → 安全誤放」段落中，找到以 **git / gh 全域選項是攻擊面** 開頭的項目。在該項目的危險旗標列舉中，把 `--ext-diff` 之後補上 `--help`，並在該項目之後新增一個同層級的項目：
...
```bash
git add CLAUDE.md
git commit -m "docs: sync CLAUDE.md with git rest-argument scanning

Records the new attack surface notes: dynamic tokens after a git subcommand,
