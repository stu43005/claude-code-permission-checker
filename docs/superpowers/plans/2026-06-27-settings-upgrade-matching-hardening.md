...
Run: `deno task test`
（即 `deno test --allow-run --allow-env --allow-read --allow-sys=uid`；`--allow-sys=uid` 為 `main_test.ts`
in-process 呼叫 `Deno.uid()` 所需，勿用缺旗標的裸 `deno test`。）
...
（`main_test.ts` 為子行程 e2e，且 in-process 呼叫 `Deno.uid()`，故需 `--allow-run` 與 `--allow-sys=uid`。）
...
- [ ] **Step 5: 更新「新增 / 修改指令規則」的「不要重複處理」指引（修正「進 rule 前已判完」）**

於 `CLAUDE.md` 找到以 `**不要重複處理**中央前置規則已涵蓋的事` 開頭、到 `——進 rule 前已判完。` 結束的句子（跨兩行），整句替換為（修正順序敘述、補第四條 `<`）：

```markdown
**不要重複處理**中央前置規則已涵蓋的事（cwd 範圍、寫入重導向、賦值前綴、範圍外 `<`）——這四項由中央前置
...
接著確認本次變更**未改動** `src/rules/`（唯二 deny 來源 `find.ts`/`factory.ts` 維持不變）。各 Task 的 commit 步驟僅 `git add` 指定檔案（`classify.ts` / `classify_test.ts` / `main_test.ts` / `CLAUDE.md` / DRAFT），從不含 `src/rules/`，故無 `src/rules/` 變更會被提交；下列三項確認工作區、暫存區、以及相對 HEAD 皆無 `src/rules/` 殘留改動：
```bash
git diff --name-only -- src/rules/         ; echo "unstaged_exit=$?"
git diff --cached --name-only -- src/rules/ ; echo "staged_exit=$?"
git diff --name-only HEAD -- src/rules/     ; echo "vsHEAD_exit=$?"
```
Expected: 三者皆**無輸出**。若任一有輸出 → 違反「只動 classify.ts」前提，停止並回報。
