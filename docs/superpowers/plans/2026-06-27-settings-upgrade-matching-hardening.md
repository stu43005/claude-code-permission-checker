...
- **Modify (test):** `src/engine/classify_test.ts` — 翻轉 1 個既有測試（範圍外 `<` 改為不可升級）、新增中央前置不可升級 / 不變量 8 / 可升級不退化測試。
...
Deno.test("輸入重導向（範圍外 <）為不可升級中央前置：Bash() 不升級、維持 ask", () => {
  // 行為變更（spec 2026-06-27 §4.4）：範圍外 < 是中央前置安全 ask，permissions.allow 不可解除
...
- [ ] **Step 3: 新增「不變量 8（rule allow 被中央前置覆寫）」與「可升級不退化」測試**

接續於 `src/engine/classify_test.ts` 檔案尾端新增：

```ts
Deno.test("不變量 8：指令規則 allow 被中央前置覆寫、不洩漏成 allow", () => {
 ...
 *
 * 一次性稽核（見 spec §4.6.1 / 不變量 9）：指令規則的 evaluate 皆為純函式、無副作用，
 * 故步驟 2 在「危險 cwd/redirect」情境下呼叫 rule.evaluate 無 runtime 危害；其 allow/ask
 * 一律被本前置覆寫（唯一能越過的是 rule 的 deny，安全方向）。
 */
...
## Task 3: 指令規則純函式稽核（緩解 spec §4.6.1 residual）

**Files:**
- Modify: `src/engine/classify.ts`（僅在 `centralPreflightAsk` 上方註解補一行稽核日期；註解框架已於 Task 1 Step 5 置入）

- [ ] **Step 1: 以 grep 稽核所有指令規則無副作用 API**
...
```
Expected: **無任何輸出**（exit code 1）。代表 `src/rules/`（含 `commands/*.ts` 與 `factory.ts`）的所有規則 `evaluate` 皆未呼叫檔案系統 / 子行程 API，符合不變量 9 純函式契約。

- [ ] **Step 2: 若 Step 1 有輸出 → 停止並回報**

若出現任何匹配：該規則違反純函式契約，屬 residual 假設破口。**停止實作**，回報使用者（此情況下需重新評估是否改採 two-phase）。若無輸出，繼續 Step 3。

- [ ] **Step 3: 在 `classify.ts` 補稽核日期註解**

於 `src/engine/classify.ts` 的 `centralPreflightAsk` doc 註解中，把該行：

```ts
 * 一次性稽核（見 spec §4.6.1 / 不變量 9）：指令規則的 evaluate 皆為純函式、無副作用，
```

替換為（加上稽核日期與範圍，便於日後查核）：

```ts
 * 一次性稽核（2026-06-27，見 spec §4.6.1 / 不變量 9）：已 grep 確認 src/rules/（commands/*.ts
 * 與 factory.ts）所有 evaluate 皆無副作用 API（Deno.write/run/Command/... 無命中），為純函式；
```

- [ ] **Step 4: 重新型別檢查 + lint（註解變更不影響行為，但保持紀律）**
...
```bash
git add src/engine/classify.ts
git commit -m "docs(classify): record rule-purity audit for preflight ordering residual

One-time audit per spec 4.6.1: grep confirms no command rule evaluate has
side-effecting Deno FS/subprocess APIs, satisfying invariant 9.
...
於 `CLAUDE.md` 找到描述 `classify.ts` 「分兩層」的那條 bullet（以 `**`classify.ts`** 分兩層` 開頭），整條替換為反映新流程的描述：

```markdown
- **`classify.ts`** 單一有序流程（合併前述「兩層」）：① `name` 為 null（動態）→ 不可升級 ask；
  ② 評估指令規則，其硬 `deny`（遞迴遍歷磁碟根/家目錄根）最優先短路、不經中央前置與升級層；
  ③ **四條中央前置規則**（見下）對**所有指令**（含未列入 allowlist 者）通用、命中即回**不可升級** ask；
  ④ 可升級 ask（未列入 allowlist、或指令規則自身 ask）才呼叫 `settingsAllows` 嘗試以 `permissions.allow`
  升級為 `allow`（命中 allow 且未被 deny/ask 命中才升級）；⑤ 指令規則 `allow` → allow。純函式
  `centralPreflightAsk` 封裝步驟 ③。**中央前置 ask 永不進升級層**（spec 2026-06-27）。
```

- [ ] **Step 2: 更新「四條中央前置規則」段落開頭描述**

於 `CLAUDE.md` 找到 `### 四條中央前置規則` 標題下、以 ``classifyBuiltin` **先評估個別指令規則**` 開頭的段落，整段替換為：
...
（中央前置覆寫 rule 的 allow/ask，唯一越過的是 rule deny；rule.evaluate 為純函式，見 spec §4.6）：
```
...
```bash
git add CLAUDE.md
git commit -m "docs(CLAUDE): document universal non-upgradable central preflight flow
...
## Task 5: Build + Operational verification（改規則後必做）
...
- `deno task build` 成功；Task 5 三項 operational verification 結果分別為 ask / allow / ask。
