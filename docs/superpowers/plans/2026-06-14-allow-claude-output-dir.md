...
Deno.test("G5(a) 負向：basename 與 session_id 不符 → []", () => {
...
Deno.test("sid 安全段負向（path-escape）：. / .. / 含分隔符 / 含點 → []", () => {
  // sid="."：transcript basename 必須 == ".jsonl" 才會試圖串接，但 sid 不符安全段先被擋
...
export function sessionTrustedReadRoots(
  ...
  if (posixBasename(abs) !== sid + ".jsonl") return []; // G5(a) session 綁定

  const dir = posixDirname(abs);
  if (dir === null) return [];

  const projectsRoot = normalizeAbsolute(home + "/.claude/projects");
  if (!isWithin(projectsRoot, dir) || dir === projectsRoot) return []; // G5(b) 位置綁定
  ...
  // Post-construction 不變量（defense-in-depth）：每個 root 的 basename 必為 sid。
...
在 `src/engine/classify_test.ts` 末尾新增（沿用檔頭既有的 `parseCommand`/`walk`/`classify`/`parseBashRule`/`parsePathRule`/`EMPTY_READ_SCOPE` 匯入與 `ROOT`/`START`/`rulesOf`）：
...
Deno.test("N4：同專案 memory / 他 session 不在 trusted → ask", () => {
...
Deno.test("G3：trusted 下但命中 user Read() deny、且無 Bash allow → ask", () => {
...
於 `src/engine/classify.ts` 找到 `classify`：
...
Deno.test("e2e: transcript basename 與 session_id 不符 -> ask（G5(a)）", async () => {
  const out = await runHookWithEnv(
    e2ePayload(`cat ${E2E_HOME}/.claude/projects/${E2E_E}/${E2E_SID}/tool-results/x.txt`, {
...
Deno.test("e2e: 同專案 memory -> ask（N4，不自動放行）", async () => {
  const out = await runHookWithEnv(
...
echo "{\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"cat $H/.claude/projects/$E/$SID/tool-results/x.txt\"},\"cwd\":\"$PJ\",\"session_id\":\"$SID\",\"transcript_path\":\"$H/.claude/projects/$E/$SID.jsonl\"}" \
  | HOME="$H" CLAUDE_PROJECT_DIR="$PJ" ./dist/permission-checker
# 2) /tmp 任務輸出 -> allow（macOS 用 /private/tmp）
echo "{\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"cat /private/tmp/claude-$UID/$E/$SID/tasks/x.output\"},\"cwd\":\"$PJ\",\"session_id\":\"$SID\",\"transcript_path\":\"$H/.claude/projects/$E/$SID.jsonl\"}" \
  | HOME="$H" CLAUDE_PROJECT_DIR="$PJ" ./dist/permission-checker
# 3) 同專案 memory -> ask（N4）
echo "{\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"cat $H/.claude/projects/$E/memory/n.md\"},\"cwd\":\"$PJ\",\"session_id\":\"$SID\",\"transcript_path\":\"$H/.claude/projects/$E/$SID.jsonl\"}" \
  | HOME="$H" CLAUDE_PROJECT_DIR="$PJ" ./dist/permission-checker
# 4) session_id 不符 -> ask（G5(a)）
...
Expected：(1) allow、(2) allow、(3) ask、(4) ask；全部 `exit 0`。
