...
export function sessionTrustedReadRoots(
  ...
  // 且每個 root 的 basename 必為 sid。step 3 安全段檢查已保證此恆成立，此處僅防未來建構邏輯回歸。
...
echo "{\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"cat /private/tmp/claude-$UID/$E/$SID/tasks/x.output\"},\"cwd\":\"$PJ\",\"session_id\":\"$SID\",\"transcript_path\":\"$T\"}" \
  | HOME="$H" CLAUDE_PROJECT_DIR="$PJ" ./dist/permission-checker
# 2b) /tmp 任務輸出（另一形式）-> allow
echo "{\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"cat /tmp/claude-$UID/$E/$SID/tasks/x.output\"},\"cwd\":\"$PJ\",\"session_id\":\"$SID\",\"transcript_path\":\"$T\"}" \
  | HOME="$H" CLAUDE_PROJECT_DIR="$PJ" ./dist/permission-checker
...
Expected：(1) allow、(2a) allow、(2b) allow、(3) ask、(4) ask；全部 `exit 0`。
